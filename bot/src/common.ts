...
export const getUserList = async (userId) => {
  ...
  request.query(`	SELECT U.EMAIL, U.FullNameKR, AU.AppId, AU.UPN, AU.AppUserId FROM
	(select EMAIL, FullNameKR
	FROM IAM.dbo.USER_ENTITY
	WHERE EMAIL is not NULL
	AND FullNameKR is not NULL) U
	LEFT OUTER JOIN 
	(select AppId, UPN, AppUserId
	FROM IAM.bot.App_User
	WHERE AppId = @botId) AU
	ON U.EMAIL = AU.UPN
	ORDER BY U.FullNameKR ASC`, (err, result) => {
    if(err){
        return console.log('query error :',err)
    }
  });
...
export const insertLog = async (userId, body) => {
  ...
  request.query(`INSERT INTO IAM.bot.App_Log
    (TS, AppId, UPN, Data)
    VALUES
    (@ts, @botId, @upn, @body)`
    , (err) => {
...
export const sendWorkplace = async (userId, func) => {
  ...
  request.on('done', result => {
    func(userId, choiceList);
  });
}

export const findWorkplace = async (userId, choiceList) => {
  const request = new sql.Request();
  request.input('botId', sql.VarChar, process.env.BOT_ID);
  request.input('date', sql.VarChar, getToday(null));
  let query = `
	SELECT AU.AppUserId, W.UPN, W.WorkCodeAM, W.WorkCodePM FROM
	(select AppId, UPN, AppUserId
	FROM IAM.bot.App_User
	WHERE AppId = @botId) AU
	LEFT OUTER JOIN 
  ...
  if(userId !== undefined && userId !== null) {
    query += ' WHERE AU.AppUserId = @userId ';
    request.input('userId', sql.VarChar, userId);
  } else {  //오전 오후가 둘 다 휴무인 사람에게는 전체 메세지를 전송하지 않음.
    query += " WHERE  NOT(W.WorkCodeAM = 'WRK-OFF' AND W.WorkCodePM = 'WRK-OFF') ";
  }
  ...
  request.on('row', (row) => {    
    sendWorkplaceCard(row.AppUserId, choiceList, row.WorkCodeAM, row.WorkCodePM);
  });
}

export const notFoundWorkplace = async (userId, choiceList) => {
  const request = new sql.Request();
  request.input('botId', sql.VarChar, process.env.BOT_ID);
  request.input('date', sql.VarChar, getToday(null));
  ...
  request.on('row', (row) => {    
    sendWorkplaceCard(row.AppUserId, choiceList, null, null);
  });
}

const sendWorkplaceCard = async (userID, choiceList, WorkCodeAM, WorkCodePM) => {
  const user = userMap[userID];
...
export const insertWorkplace = async (userId, body) => {
  ...
  request.input("WorkDate", sql.VarChar, body.WorkDate) ;
  request.input('UPN', sql.VarChar, userPrincipalName);
  request.input('WorkCodeAM', sql.VarChar, body.WorkCodeAM);
  request.input('WorkCodePM', sql.VarChar, body.WorkCodePM);

  request.query(`IF EXISTS (SELECT 1 FROM IAM.bot.Workplace WHERE WorkDate = @WorkDate AND UPN = @upn)
  BEGIN
    UPDATE IAM.bot.Workplace
    SET WorkCodeAM=@WorkCodeAM, WorkCodePM=@WorkCodePM, SaveDate=GETDATE()
    WHERE UPN = @UPN AND WorkDate=@WorkDate;
  END
  ELSE
  BEGIN
INSERT INTO IAM.bot.Workplace
(WorkDate, UPN, WorkCodeAM, WorkCodePM)
VALUES
(@WorkDate, @UPN, @WorkCodeAM, @WorkCodePM)
  END`
    , (err) => {
